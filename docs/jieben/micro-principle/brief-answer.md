...
3. 辅助存储用来扩大存储容量
4. 主存储器，计算机运行所用到的指令和数据都存放在主存储器中
### 半导体存储器的主要性能指标
...
- 当微处理器和存储器交换信号时，首先由CPU发送存储器的地址，同时发允许锁存信号ALE给锁存器，锁存器接收到信号后，将地址数据、状态总线上的地址锁存在锁存器中，随后太可以传送数据信息。
## 中断技术
### 什么是中断，使用中断的好处？
...
1. 无条件传送：不查询外设状态，认为外设已经准备就绪，直接与外设传送数据。(硬件最少)
2. 查询传送：CPU不断查询外设端口的状态，状态不符合时候，CPU需要等待，直到状态符合时，CPU完成相应操作。CPU利用率低
3. 中断控制：外设在准备就绪的条件下通过请求CPU，主动向CPU提出交换数据，CPU与外设之间能并行工作。(每十秒进行数据采集)
