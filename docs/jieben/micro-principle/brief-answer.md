...
### 物理地址、逻辑地址、偏移地址
1. 实际情况下，每个存储单元分配一个唯一的20位的二进制数（5位的十六进制数）的存储器地址，称为物理地址。
2. 逻辑地址是由16为二进制数(4位十六进制数)的段地址和偏移地址组成
3. 段地址是指CS，DS，SS，ES存放这些地址的高4位，则某个段寄存器便会存储高4位信息，即为段地址。
4. 段内偏移地址就是移动后相对于段地址的偏移量
5. 段地址*16+偏移地址即是物理地址
...
2. 工作方式
-  方式0-计数结束中断
-  方式1-硬件触发单拍脉冲   等待GATE(门控信号)
-  方式2-频率发生器（周期） 负脉冲
-  方式3-方波发生器（周期） 计算过程中输出有一半时间是高电平，一半时间是低电平
-  方式4-软件触发选通   负脉冲
-  方式5-硬件触发选通   负脉冲  等待GATE(门控信号) 
3. init
