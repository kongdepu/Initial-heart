...
### 缓冲寄存器
1. 完成速度的匹配，在高速工作的CPU与慢速工作的外设间起协调和缓冲的作用，实现数据传送的同步
...
## 中断技术
### 什么是中断，使用中断的好处？
1. 所谓中断，是指计算器在正常运行执行中，由于种种原因，是CPU暂时停止处理当前程序的执行，而去转向临时发生的时间，处理完毕后，在返回去处理执行暂停的程序
2. 好处
...
3. 保护现场
- 8086要把用到的寄存器的内容用PUSH指令推入堆栈，而标志位的状态实在保留断点的同时由硬件推入堆栈的
